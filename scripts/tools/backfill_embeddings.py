...
async def fetch_missing(
    ...
    while True:
        select_clause = "select=id,lesson,unit,question,explanation"
...
async def _patch_one(
    ...
    payload = {COLUMN_NAME: emb}                     # list direkt JSON array → pgvector kabul eder
...
async def backfill(
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for batch_idx, i in enumerate(range(0, total, BATCH_SIZE), start=1):
            ...
            texts = [
                f"{q.get('lesson', '')} {q.get('unit', '')}: {q.get('question', '')}\n{q.get('explanation', '')[:200]}".strip()
                for q in chunk
            ]
