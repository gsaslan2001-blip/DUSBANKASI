...
-- ─── 9. active_sessions için auth-bazlı RLS ──────────────────────────
-- Mevcut public politikaları kaldır
DROP POLICY IF EXISTS "Public read sessions"   ON active_sessions;
DROP POLICY IF EXISTS "Public insert sessions" ON active_sessions;
DROP POLICY IF EXISTS "Public update sessions" ON active_sessions;
DROP POLICY IF EXISTS "Public delete sessions" ON active_sessions;

CREATE POLICY "Read own session" ON active_sessions
  FOR SELECT USING (
    (auth.uid() IS NOT NULL AND auth.uid() = user_id) OR
    (auth.uid() IS NULL     AND user_id IS NULL)
  );

CREATE POLICY "Insert own session" ON active_sessions
  FOR INSERT WITH CHECK (
    (auth.uid() IS NOT NULL AND auth.uid() = user_id) OR
    (auth.uid() IS NULL     AND user_id IS NULL)
  );

CREATE POLICY "Update own session" ON active_sessions
  FOR UPDATE USING (
    (auth.uid() IS NOT NULL AND auth.uid() = user_id) OR
    (auth.uid() IS NULL     AND user_id IS NULL)
  );

CREATE POLICY "Delete own session" ON active_sessions
  FOR DELETE USING (
    (auth.uid() IS NOT NULL AND auth.uid() = user_id) OR
    (auth.uid() IS NULL     AND user_id IS NULL)
  );

-- ─── 10. Realtime için tabloları publish et ──────────────────────────
