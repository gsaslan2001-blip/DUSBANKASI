...
export function SourceBooksView({ questions, onBack, theme }: SourceBooksViewProps) {
  ...
  const handleDelete = async (source: ReferenceSource) => {
    ...
    try {
      ...
      setError(err?.message || 'Silinemedi. Lütfen tekrar deneyin.');
    } finally {
      setDeletingIds((prev) => {
        ...
        <div className={`col-span-1 ${theme.card} p-5 rounded-2xl flex flex-col overflow-y-auto`}>
          ...
          <div className="space-y-3.5">
            ...
            <div>
              ...
              <div
                className={`border border-dashed rounded-xl p-5 text-center relative group transition-colors ${
                  isDragging
                ...
                onDrop={(e) => {
                  ...
                  if (dropped) {
                    if (dropped.size > MAX_FILE_BYTES) {
                      setError(`Dosya çok büyük (${(dropped.size / 1024 / 1024).toFixed(1)} MB). Maksimum 100 MB.`);
                      return;
                    }
                    ...
                    {confirmDeleteId === source.id ? (
                      <div className="flex items-center gap-1.5 shrink-0">
                        <button
                          ...
                          className="px-2 py-1 rounded-md bg-red-500/20 text-red-400 text-[11px] font-bold hover:bg-red-500/30 transition-all"
                        >
                          Sil
                        </button>
                        <button
                          onClick={() => setConfirmDeleteId(null)}
                          className={`px-2 py-1 rounded-md ${theme.inputBg} text-[11px] font-medium transition-all`}
                        >
                          İptal
                        </button>
                      </div>
