...
export const QuestionCard = memo(forwardRef<HTMLDivElement, QuestionCardProps>(function QuestionCard(
  ...
  const cleanedQuestion = useMemo(() => {
    let cleanText = question.question.split('\n').filter(l => !OPTION_LINE_RE.test(l.trim())).join('\n');
    const romanNumeralRe = /(?:\s|^)(I{1,3}|IV|V|VI{1,3}|IX|X)\.\s/g;
    return cleanText.replace(romanNumeralRe, (_, p1) => {
      return `\n\n${p1}. `;
    }).trim();
  }, [question.question]);
  ...
  return (
    <div ref={ref} className={`flex-1 flex flex-col overflow-y-auto ${theme.card} p-5 sm:p-6 rounded-2xl custom-scrollbar overflow-x-hidden`}>
      <div className="flex items-start justify-between gap-4 mb-6">
        <p className={`${fontSizes[settings.fontSize]} leading-relaxed font-medium transition-[font-size,line-height] duration-300 whitespace-pre-line`}>
          {cleanedQuestion}
        </p>
        <button
      ...
      <div className="space-y-2">
        {optionEntries.map(([key, text]) => {
          ...
          else cl += `border-transparent ${settings.theme === 'light' ? 'opacity-30' : 'opacity-20'}`;
          return (
