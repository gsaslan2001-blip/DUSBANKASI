  ...
  return (
    <div className="flex-1 flex flex-col gap-5 anim-slide-up pb-10 max-w-3xl mx-auto w-full">
      ...
      <div className="flex flex-col sm:flex-row gap-3 w-full">
        <button onClick={onRestart} className={`${incorrect > 0 && onRetryIncorrect ? 'btn bg-white/5 hover:bg-white/10 flex-1 text-sm font-bold rounded-2xl py-4' : 'btn btn-primary btn-lg w-full shadow-lg shadow-indigo-500/15'}`}>
          ANA EKRANA DÖN
        </button>
        {incorrect > 0 && onRetryIncorrect && (
          <button onClick={onRetryIncorrect} className="btn btn-primary flex-1 text-sm font-bold rounded-2xl py-4 shadow-lg shadow-indigo-500/15">
            {incorrect} YANLIŞI TEKRAR ÇÖZ →
          </button>
        )}
      </div>
    </div>
